...
const loadNoteContent = () => {
  if (noteId === 'hello-world') {
    ...
    noteContent.value = `	      <h1>Java 入门：从 Hello World 到理解 JVM 运行原理</h1>
	      ...
	      <ol>
	          ...
	          <li><strong>新建系统变量</strong>：变量名 <code>JAVA_HOME</code>，变量值是 JDK 安装路径（例如 <code>C:\Program Files\Eclipse Adoptium\jdk-21.0.2.13-hotspot\</code>）。这步不是操作系统必需的，但 Maven、Gradle、Tomcat、IDE 等工具都会读取 <code>JAVA_HOME</code> 来定位 JDK，<strong>所以强烈建议配置</strong>。</li>
	          <li><strong>编辑 Path 变量</strong>：新增一条 <code>%JAVA_HOME%in</code>（注意不要删掉 Path 里已有的其他内容）。</li>
...
java HelloWorld         # 运行：字节码 → 机器码 → 执行</code></pre>
	      ...
	      <table style="width:100%; border-collapse: collapse; margin-bottom: 1em;">
	          ...
	          <tr>
	              ...
	              <td style="border: 1px solid #ccc; padding: 8px;">检查 Path 是否包含 <code>%JAVA_HOME%in</code>，重新打开 CMD</td>
	          </tr>
	          <tr>
	      ...
	      <p>编程学习就像拼拼图——每一块看似零散的知识，最终都会连成一片完整的版图。<strong>不急不躁，把基础打牢，我们下一篇文章见。</strong></p>`;
  } else if (noteId === 'idea-shortcuts') {
    noteTitle.value = 'IDEA Shortcuts';
    noteContent.value = `	      <h1>IntelliJ IDEA 高效开发：从快捷键到生产力飞跃</h1>
...
inn + Tab     →  if (var != null) {}</code></pre>
    ...
    noteContent.value = `	      <h1>Java 集合框架深度解析：从 ArrayList 到 HashMap 底层原理</h1>
...
list.removeIf(s -> s.length() < 3);</code></pre>
	      ...
	      <h2>小结</h2>
	      <p>集合框架是 Java 最常用的 API，掌握好它有三个层次：第一层是会 CRUD 操作；第二层是理解底层数据结构，能根据场景正确选型；第三层是理解扩容机制、线程安全问题，能写出高性能代码。<strong>大部分开发者停留在第一层，你至少要到第二层——选对集合，很多性能问题根本不会出现。</strong></p>`;
  } else if (noteId === 'vue-migration') {
    noteTitle.value = 'Vue Migration';
    noteContent.value = `	      <h1>网站重构实战：从静态 HTML 到 Vue 3 + TypeScript 的现代化改造</h1>
...
npm run dev    # 启动开发服务器，浏览器自动打开 http://localhost:5173</code></pre>`;
    ...
    noteContent.value = `	      <h1>Spring Boot 快速入门：从零搭建你的第一个 RESTful API</h1>
...
java -jar target/demo-0.0.1-SNAPSHOT.jar --server.port=9090</code></pre>
	      ...
	      <p>接下来值得深入的方向：Spring Security（认证授权）、Spring Data JPA（持久层）、Spring Cloud（微服务）、以及自动化测试（单元测试 + 集成测试）。</p>`;
  } else if (noteId === 'mysql-basics') {
    noteTitle.value = 'MySQL Basics';
    noteContent.value = `	      <h1>MySQL 从入门到高效使用：SQL 语法、设计规范与性能优化</h1>
...
-- 关注 rows 列：预估扫描行数</code></pre>
	      ...
	      <p>下一步推荐学习的方向：慢查询日志分析（<code>slow_query_log</code>）、EXPLAIN 执行计划详解、分库分表方案（ShardingSphere）、以及 MySQL 8.0 的新特性（窗口函数、CTE 递归查询等）。</p>`;
  } else if (noteId === 'git-basics') {
    noteTitle.value = 'Git 常用命令';
    noteContent.value = `	      <h1>Git 从入门到团队协作：常用命令、工作流与踩坑指南</h1>
...
Thumbs.db              # Windows 系统文件</code></pre>
	      ...
	      <h2>小结</h2>
	      <p>Git 的命令很多，但 80% 的时间你只需要：<strong>status、add、commit、push、pull、branch、checkout、merge</strong> 这 8 个命令。其余命令是"救火"用的——遇到问题时知道有对应解决方案就行，不必死记硬背。<strong>最好的学习方式是在真实项目中使用 Git</strong>，遇到冲突不要怕，这正是理解 Git 原理的好机会。</p>`;
  } else if (noteId === 'docker-intro') {
    noteTitle.value = 'Docker 入门指南';
    noteContent.value = `	      <h1>Docker 容器化实战：从镜像构建到多容器编排</h1>
...
docker-compose ps                 # 查看所有服务状态</code></pre>
	      ...
	      <p>下一步推荐学习：Docker 镜像优化和安全性（镜像扫描、签名）、Kubernetes 基础（Pod、Service、Deployment）、CI/CD 中的 Docker 集成（GitHub Actions + Docker Build）。</p>`;
  } else if (noteId === 'linux-commands') {
    noteTitle.value = 'Linux 基础命令';
    noteContent.value = `	      <h1>Linux 基础命令速查：从文件操作到进程管理</h1>
...
sed -i 's/old-text/new-text/g' *.txt     # 当前目录所有 .txt 文件</code></pre>
	      ...
	      <h2>小结</h2>
	      <p>Linux 命令不需要死记硬背——<strong>记住最常用的 20 个</strong>（ls、cd、cat、less、tail、grep、find、chmod、ps、top、kill、df、du、free、ping、curl、ssh、scp、tar、vim），其余的遇到时查 <code>man</code>（manual）或 <code>--help</code> 就行。最重要的是<strong>理解管道的理念</strong>——通过组合简单命令来完成复杂任务，这是 Linux 命令行真正的威力所在。</p>`;
  } else if (noteId === 'javascript-es6') {
    noteTitle.value = 'JavaScript ES6+ 新特性';
    noteContent.value = `	      <h1>JavaScript ES6+ 常用新特性：从回调地狱到优雅的 Async/Await</h1>
...
[1, [2, [3]]].flat(2);            // [1, 2, 3]（扁平化指定层数）</code></pre>
    ...
    noteContent.value = `	      <h1>设计模式入门：从单例到观察者，写出更优雅的代码</h1>
...
// 完全不用改 UserService 的代码——这就是观察者模式的威力。</code></pre>
	      ...
	      <h2>小结</h2>
	      <p>设计模式不是背出来的，是<strong>在写代码的过程中"悟"出来的</strong>。当你写了一段代码感觉"这里好像不太好改"、"这个类好像太大了"时，去看看设计模式——大概率正好有一种模式能解决你的问题。不需要一次学完 23 种，<strong>先掌握单例、工厂、建造者、策略、观察者这 5 种最常用的</strong>，其余的随着实践慢慢补充。</p>`;
  } else if (noteId === 'redis-basics') {
    noteTitle.value = 'Redis 基础入门';
    noteContent.value = `	      <h1>Redis 缓存实战：从数据结构到缓存击穿解决方案</h1>
